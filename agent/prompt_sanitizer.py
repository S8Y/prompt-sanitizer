    ...
    def _sanitize_secrets(self, text: str) -> str:
        ...
        if "=" in text:
            def _env_replace(m):
                ...
                now = time.time()
                self._vault[placeholder] = {
                    "value": f"{name}={quote}{value}{quote}",
                    "created_at": now,
                    "last_used_at": now,
                }
        ...
        if re.search(r"(?:password|passwd|secret|api_key|apikey|auth_token|access_token|refresh_token|private_key|secret_key|api_secret)\s*[=:]", text, re.IGNORECASE):
            def _cred_replace(m):
                ...
                # and Python/JSON literal non-secrets (None, null, true, false)
                if re.match(r"^(\*{3,}|\[(FILTERED|REDACTED|HIDDEN)\]|\.{3,}|None|null|nil|true|false|undefined|nullptr)$", value, re.IGNORECASE):
                    return m.group(0)
                return self._sanitize_match(m, "CREDENTIAL")
        ...
        if ":" in text:
            def _telegram_replace(m):
                ...
                now = time.time()
                self._vault[placeholder] = {
                    "value": f"{bot_prefix}{digits}:{token}",
                    "created_at": now,
                    "last_used_at": now,
                }
        ...
        **Standalone domain handling** — bare FQDNs (3+ labels like
            ...
            def _url_hostname_replace(m):
                ...
                try:
                    ...
                    if any(
                        ...
                        # Safe domain: return URL with zero-width spaces in hostname
                        # dots to prevent _DOMAIN_RE from matching subdomains
                        # in the second pass. ZWS is invisible to humans and LLMs.
                        protected_netloc = parsed.netloc
                        lowered_netloc = protected_netloc.lower()
                        idx = lowered_netloc.find(hostname_lower)
                        if idx >= 0:
                            protected_host = protected_netloc[idx:idx+len(hostname)]
                            protected_host = protected_host.replace('.', '.\u200b')
                            protected_netloc = (
                                protected_netloc[:idx]
                                + protected_host
                                + protected_netloc[idx+len(hostname):]
                            )
                        scheme = parsed.scheme or "https"
                        return f"{scheme}://{protected_netloc}{parsed.path or ''}" + (
        ...
        if "." in text:
            def _domain_replace(m):
                ...
                last_dot = domain.lower().rfind(".")
                if last_dot != -1 and _valid_tld(domain.lower()[last_dot + 1:]):
